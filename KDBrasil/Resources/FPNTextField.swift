...
open class FPNTextField: UITextField, CountryPickerDelegate, FPNDelegate {
	...
	private func setupCountryPicker() {
		...
		if let regionCode = Locale.current.regionCode, let countryCode = CountryCode(rawValue: regionCode) {
	...
	public func setFlag(for countryCode: CountryCode) {
		countryPicker.setCountry(countryCode)
	}

	/// Get the current formatted phone number
	public func getFormattedPhoneNumber(format: PhoneNumberFlagFormat) -> String? {
	...
	public func set(phoneNumber: String) {
		...
		if let validPhoneNumber = getValidNumber(phoneNumber: cleanedPhoneNumber) {
			...
			setFlag(for: CountryCode(rawValue: phoneUtil.getRegionCode(for: validPhoneNumber))!)
		}
	}

	/// Set the country list excluding the provided countries
	public func setCountries(excluding countries: [CountryCode]) {
		countryPicker.setup(without: countries)
	}

	/// Set the country list including the provided countries
	public func setCountries(including countries: [CountryCode]) {
	...
	private func convert(format: PhoneNumberFlagFormat) -> NBEPhoneNumberFormat {
		switch format {
		case .E164:
	...
	internal func fpnDidSelect(country: Country) {
